---
name: verify
description: lol-mon「LoLもん」(React Router SPA) をローカルで起動し、ブラウザでクイズフローを一通り駆動して検証する手順
---
...
1. `/` — レーン 5 個 + 出題タイプ 8 個のチェックボックス（デフォルト全チェック）と
   「クイズを開始する」ボタン。タイプ 0 個、またはチャンピオン系タイプありでレーン 0 個
...
- GitHub Pages 相当のビルド確認: `GITHUB_REPOSITORY=lunastera/lol-mon npm run build`
  → `build/client/index.html` のアセットパスが `/lol-mon/` 始まり、`404.html` が存在すること。
